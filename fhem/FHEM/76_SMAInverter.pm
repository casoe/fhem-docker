#################################################################################################################
# $Id: 76_SMAInverter.pm 28031 2023-10-08 13:56:55Z MadMax $
#################################################################################################################
#
#  Copyright notice
#
     ...
     if(SMAInverter_SMAlogon($hash->{HOST}, $hash->{PASS}, $hash)) {
         ...
         if ($sc) {			 # SBFSpot Kompatibilitätsmodus
			 ...
			 if($sup_PVEnergyProduction) {
                 push(@row_array, "epvtotal ".($inv_SPOT_EPVTOTAL/1000)."\n")  if ($inv_SPOT_EPVTOTAL ne "-");
			 ...
			 if($sup_PVEnergyProduction) {
                 push(@row_array, "SPOT_EPVTOTAL ".$inv_SPOT_EPVTOTAL."\n") if ($inv_SPOT_EPVTOTAL ne "-");
...
sub SMAInverter_setVersionInfo($) {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {
      ...
      if($modules{$type}{META}{x_version}) {                                                                             # {x_version} ( nur gesetzt wenn $Id: 76_SMAInverter.pm 28031 2023-10-08 13:56:55Z MadMax $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/xsg;
      } else {
          $modules{$type}{META}{x_version} = $v;
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                                                                # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 76_SMAInverter.pm 28031 2023-10-08 13:56:55Z MadMax $ im Kopf komplett! vorhanden )
