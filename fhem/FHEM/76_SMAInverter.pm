#################################################################################################################
# $Id: 76_SMAInverter.pm 28469 2024-02-03 14:30:42Z MadMax $
#################################################################################################################
#
#  Copyright notice
#
...
sub SMAInverter_setVersionInfo($) {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {
      ...
      if($modules{$type}{META}{x_version}) {                                                                             # {x_version} ( nur gesetzt wenn $Id: 76_SMAInverter.pm 28469 2024-02-03 14:30:42Z MadMax $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/xsg;
      } else {
          $modules{$type}{META}{x_version} = $v;
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                                                                # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 76_SMAInverter.pm 28469 2024-02-03 14:30:42Z MadMax $ im Kopf komplett! vorhanden )
