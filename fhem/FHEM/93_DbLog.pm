############################################################################################################################################
# $Id: 93_DbLog.pm 26672 2022-11-07 19:53:34Z DS_Starter $
#
# 93_DbLog.pm
...
sub DbLog_execmemcache {
  ...
  if($hash->{HELPER}{".RUNNING_PID"} && $hash->{HELPER}{".RUNNING_PID"}{pid} =~ m/DEAD/) {   # tote PID's löschen
      delete $hash->{HELPER}{".RUNNING_PID"};
  }
  if($hash->{HELPER}{REDUCELOG_PID} && $hash->{HELPER}{REDUCELOG_PID}{pid} =~ m/DEAD/) {
      delete $hash->{HELPER}{REDUCELOG_PID};
...
sub DbLog_setVersionInfo {
  ...
  if($modules{$type}{META}{x_prereqs_src} && !$hash->{HELPER}{MODMETAABSENT}) {       # META-Daten sind vorhanden
      ...
      if($modules{$type}{META}{x_version}) {                                          # {x_version} ( nur gesetzt wenn $Id: 93_DbLog.pm 26672 2022-11-07 19:53:34Z DS_Starter $ im Kopf komplett! vorhanden )
          $modules{$type}{META}{x_version} =~ s/1\.1\.1/$v/xsg;
      } 
      else {
          $modules{$type}{META}{x_version} = $v; 
      }
      return $@ unless (FHEM::Meta::SetInternals($hash));                             # FVERSION wird gesetzt ( nur gesetzt wenn $Id: 93_DbLog.pm 26672 2022-11-07 19:53:34Z DS_Starter $ im Kopf komplett! vorhanden )
